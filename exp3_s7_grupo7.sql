...
CREATE SYNONYM syn_ticket_conc FOR TICKETS_CONCIERTO;

-- paso 3
-- simulacion
CREATE OR REPLACE VIEW SIMULACION_ANTIGUEDAD AS
Select TO_CHAR(a.numrut, '999999999')||'-'||a.dvrut as "NUM",
UPPER(a.nombre) || ' ' || UPPER(a.appaterno) || ' ' || UPPER(a.apmaterno) AS "NOMBRE_TRABAJADOR",
TO_CHAR(a.sueldo_base, '$999G999G999')
AS "SUELDO_BASE",
CASE
WHEN (b.nro_ticket) IS NULL THEN 'No hay info'
ELSE TO_CHAR((b.nro_ticket))
END as "NUM_TICKET",
INITCAP(LOWER(a.direccion)) AS "DIRECCION",
c.nombre_isapre AS "SISTEMA_SALUD",
TO_CHAR((NVL(b.monto_ticket,0)), '$999G999G999') AS "MONTO",
TO_CHAR(NVL((CASE 
WHEN b.monto_ticket <=50000 THEN 0
WHEN b.monto_ticket BETWEEN 50001 AND 100000 THEN b.monto_ticket*0.05
WHEN b.monto_ticket >100000 THEN b.monto_ticket*0.07
END),0), '$999G999G999') AS "BONIF_POR_TICKET",
TO_CHAR(a.sueldo_base + NVL((CASE 
WHEN b.monto_ticket <=50000 THEN 0
WHEN b.monto_ticket BETWEEN 50001 AND 100000 THEN b.monto_ticket*0.05
WHEN b.monto_ticket >100000 THEN b.monto_ticket*0.07
END),0), '$999G999G999') AS "SIMULACION_POR_TICKET",
TO_CHAR(ROUND(a.sueldo_base + a.sueldo_base*d.porcentaje), '$999G999G999') AS "SIMULACION_POR_ANTIGUEDAD"

from syn_trabajador a
left join syn_ticket_conc b
ON a.numrut=b.numrut_t
join isapre c
ON a.cod_isapre=c.cod_isapre
join syn_bono_ant d
ON TRUNC(MONTHS_BETWEEN(SYSDATE, a.fecing) / 12) BETWEEN d.limite_inferior AND d.limite_superior
WHERE c.porc_descto_isapre>4 AND TRUNC(MONTHS_BETWEEN(SYSDATE, a.fecnac) / 12)<50

order by NVL(b.monto_ticket,-1)  DESC,"NOMBRE_TRABAJADOR" ASC;
...
SELECT
    ...
    "MONTO",
    "BONIF_POR_TICKET",
    "SIMULACION_POR_TICKET",
    "SIMULACION_POR_ANTIGUEDAD"
FROM SIMULACION_ANTIGUEDAD;
...
select * from asignacion_familiar

-- PASO 2
-- EXTRAS
CREATE SYNONYM syn_trabajador FOR TRABAJADOR;
CREATE SYNONYM syn_bono_esc FOR BONO_ESCOLAR;

-- paso 3 
-- simular vista
CREATE OR REPLACE VIEW V_AUMENTO_ESTUDIOS AS
Select TO_CHAR(a.numrut, '999999999')||'-'||a.dvrut as "RUT_TRABAJADOR",
INITCAP(LOWER(a.nombre)) || ' ' || INITCAP(LOWER(a.appaterno)) || ' ' || INITCAP(LOWER(a.apmaterno)) AS "TRABAJADOR",
b.descrip AS "DESCRIP",
TO_CHAR(b.porc_bono,'0000000') AS "PCT_ESTUDIOS",
a.sueldo_base
AS "SUELDO_ACTUAL",
ROUND(a.sueldo_base*(b.porc_bono/100)) AS "AUMENTO",
TO_CHAR(ROUND(a.sueldo_base+a.sueldo_base*(b.porc_bono/100)),'$999G999G999') AS "SUELDO_AUMENTADO"

from syn_trabajador a
join syn_bono_esc b
ON a.id_escolaridad_t=b.id_escolar
join tipo_trabajador c
ON a.id_categoria_t=c.id_categoria

WHERE a.id_categoria_t=3 OR (
        SELECT COUNT(*)
        FROM asignacion_familiar af
        WHERE af.numrut_t = a.numrut
      ) BETWEEN 1 AND 2

order by b.porc_bono ASC,"RUT_TRABAJADOR" ASC;
...
CREATE INDEX idx_trabajador_castillo
ON trabajador(
    CASE WHEN UPPER(apmaterno) = 'CASTILLO' THEN apmaterno END
);
