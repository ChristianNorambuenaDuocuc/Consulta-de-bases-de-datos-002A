...
SELECT 
        ...
        UPPER(a.nombre) || ' ' || UPPER(a.appaterno) || ' ' || UPPER(a.apmaterno) AS "Nombre Completo Trabajador",
        TO_CHAR(a.numrut, '999G999G999')||'-'||a.dvrut as "RUT Trabajador",      
        UPPER(b.desc_categoria) as "Tipo Trabajador",
        UPPER(c.nombre_ciudad)
        as "Ciudad Trabajador",   
        TO_CHAR(a.sueldo_base, '$999G999G999')
        AS "Sueldo Base"
FROM TRABAJADOR a
INNER JOIN TIPO_TRABAJADOR b
ON a.id_categoria_t=b.id_categoria
INNER JOIN COMUNA_CIUDAD c
ON a.id_ciudad=c.id_ciudad
WHERE  a.sueldo_base BETWEEN 650000 AND 3000000
ORDER BY c.nombre_ciudad desc, a.sueldo_base asc;

-- ------- CASO 2: Listado cajeros -------
SELECT 

        TO_CHAR(b.numrut, '999G999G999')||'-'||b.dvrut as "RUT Trabajador",
        INITCAP(LOWER(b.nombre)) || ' ' || UPPER(b.appaterno) AS "Nombre Trabajador",        
        COUNT(a.nro_ticket) as "Total Tickets",
        TO_CHAR(SUM(a.monto_ticket),'$999G999G999') as "Total Vendido",
        TO_CHAR(SUM(c.valor_comision),'$999G999G999') as "Comision Total",
        UPPER(d.desc_categoria) as "Tipo Trabajador",
        UPPER(e.nombre_ciudad)
        as "Ciudad Trabajador"
FROM tickets_concierto a
INNER JOIN trabajador b
ON a.numrut_t=b.numrut
INNER JOIN  comisiones_ticket c
ON a.nro_ticket=c.nro_ticket
INNER JOIN tipo_trabajador d
ON b.id_categoria_t=d.id_categoria
INNER JOIN COMUNA_CIUDAD e
ON b.id_ciudad=e.id_ciudad
WHERE  d.desc_categoria='CAJERO'
GROUP BY b.numrut,b.dvrut,INITCAP(LOWER(b.nombre)) || ' ' || UPPER(b.appaterno),UPPER(d.desc_categoria),
UPPER(e.nombre_ciudad)
HAVING SUM(a.monto_ticket)>50000
ORDER BY SUM(a.monto_ticket) desc;

-- ------- CASO 3: Listado de bonificaciones -------
SELECT 
 
        TO_CHAR(b.numrut, '999G999G999')||'-'||b.dvrut as "RUT Trabajador",
        INITCAP(LOWER(b.nombre)) || ' ' || INITCAP(LOWER(b.appaterno)) AS "Trabajador Nombre",         
        EXTRACT(YEAR FROM b.fecing) as "Año Ingreso",
        TRUNC(MONTHS_BETWEEN(SYSDATE, b.fecing) / 12) as "Años Antiguedad",
        COUNT(a.numrut_carga) as "Num. Cargas Familiares",
        INITCAP(LOWER(f.nombre_isapre)) as "Nombre Isapre",
        TO_CHAR(b.sueldo_base,'$999G999G999')
        as "Sueldo Base",
        TO_CHAR(
        CASE
         WHEN INITCAP(LOWER(f.nombre_isapre))='Fonasa' THEN MAX(b.sueldo_base)*0.01
         ELSE 0
        END ,'$999G999G999')AS "Bono Fonasa",
        TO_CHAR(
        CASE
         WHEN TRUNC(MONTHS_BETWEEN(SYSDATE, b.fecing) / 12)<11 THEN MAX(b.sueldo_base)*0.1
         WHEN TRUNC(MONTHS_BETWEEN(SYSDATE, b.fecing) / 12)>10 THEN MAX(b.sueldo_base)*0.15
        END ,'$999G999G999')AS "Bono Antiguedad",
        INITCAP(LOWER(c.nombre_afp)) as "Nombre AFP",
        UPPER(e.desc_Estcivil) as "Estado Civil"

FROM trabajador b
LEFT JOIN asignacion_familiar a
ON a.numrut_t=b.numrut
LEFT JOIN  afp c
ON b.cod_afp=c.cod_afp
INNER JOIN  est_civil d
ON b.numrut=d.numrut_t
LEFT JOIN  estado_civil e
ON d.id_estcivil_est=e.id_estcivil
LEFT JOIN  isapre f
ON b.cod_isapre=f.cod_isapre
WHERE   (d.fecter_estcivil IS NULL OR d.fecter_estcivil>SYSDATE)
GROUP BY b.numrut,b.dvrut,INITCAP(LOWER(b.nombre)) || ' ' || INITCAP(LOWER(b.appaterno)),
EXTRACT(YEAR FROM b.fecing),TRUNC(MONTHS_BETWEEN(SYSDATE, b.fecing) / 12),
INITCAP(LOWER(f.nombre_isapre)),b.sueldo_base,INITCAP(LOWER(c.nombre_afp)),e.desc_Estcivil
ORDER BY b.numrut asc;
